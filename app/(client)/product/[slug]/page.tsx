...
async function getProductBySlugFromAPI(slug: string): Promise<Product | null> {
  try {
    const apiUrl = `${NEXT_PUBLIC_BASE_URL}/api/products?slug=${slug}`;
    const res = await fetch(apiUrl, { next: { revalidate: 60 } });
    if (!res.ok) {
      console.error(`Failed to fetch product ${slug}: ${res.statusText}`);
      return null;
    }
    const product = await res.json();
    return product;
  } catch (error) {
    console.error(`Error fetching product ${slug} from API:`, error);
    return null;
  }
}
